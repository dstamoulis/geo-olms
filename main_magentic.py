...
from utils import DATABASE_AGENT_USER, MAP_AGENT_USER, DETECTOR_AGENT_USER, DATA_AGENT_USER, MAGENTIC_PROMPT
...
def main(args, workflow=None, query="No query provided"):
    ...
    orch_agent = SingleAgent(
        ...
        messages=messages,
        toolsets_list=[],
        system_message=MAGENTIC_PROMPT
    )

    subagents_instructions = {
            "database_agent": DATABASE_AGENT_USER,
            "detector_agent": DETECTOR_AGENT_USER,
            "map_agent": MAP_AGENT_USER,
            "data_agent": DATA_AGENT_USER
    }
    ...
    response = platform.agent.run_magentic_stateflow(
        query=query,
        agents={
        ...
        orch_instructions = MAGENTIC_PROMPT,
...
if __name__ == "__main__":
    ...
    parser.add_argument('--agent', default= 'magentic', help='agent to use')
